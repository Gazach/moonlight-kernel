...
// scancode definitions for Set 1
...
// in Set 1, release = scancode | 0x80
#define IS_RELEASE(sc) ((sc) & 0x80)
#define RELEASE_KEY(sc) ((sc) & ~0x80)
...
static unsigned char unshifted[][2] = {
    ...
    {KEY_SPACE,' '},{KEY_TAB,'\t'},{KEY_BACKSPACE,'\b'},{KEY_ENTER,'\n'},
    {0,0}
};
...
static void keyboard_buffer_put(char c) {
    if (keyboard_buffer_count >= KEYBOARD_BUFFER_SIZE) return;
...
static void keyboard_handler(void) {
    ...
    if (IS_RELEASE(scancode)) {
        uint8_t key = RELEASE_KEY(scancode);
